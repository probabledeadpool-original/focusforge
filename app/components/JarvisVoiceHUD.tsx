...
export default function JarvisVoiceHUD() {
  ...
  // =========================================================================
  // MINIMIZED MODE: APPLE INTELLIGENCE GLASS BUBBLE CAPSULE
  // =========================================================================
  if (displayMode === 'minimized') {
    return (
      <AnimatePresence>
        <motion.div
          key="jarvis-minimized-bubble"
          initial={{ opacity: 0, scale: 0.8, y: -20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.8, y: -20 }}
          transition={{ duration: 0.3, ease: [0.16, 1, 0.3, 1] }}
          className="fixed top-5 right-6 z-[600] flex items-center gap-2 select-none group/bubble"
        >
          {/* Main Glass Sphere Button */}
          <div
            onClick={(e) => {
              e.stopPropagation();
              if (voiceState === 'listening_for_command' || voiceState === 'transcribing_command') {
                jarvisVoiceEngine.commitCommand();
              } else if (voiceState === 'speaking_response') {
            ...
            className="w-13 h-13 rounded-full relative flex items-center justify-center cursor-pointer shadow-[0_12px_32px_rgba(0,0,0,0.8),0_0_20px_rgba(6,182,212,0.25)] border border-white/20 hover:border-white/40 transition-transform active:scale-95 group/lens overflow-hidden bg-black/80 backdrop-blur-2xl"
          >
            {/* 1. Deep Spherical Glass Lens Base */}
            <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle_at_50%_25%,rgba(28,34,50,0.95)_0%,rgba(10,12,18,0.98)_65%,rgba(2,3,6,1)_100%)] pointer-events-none" />
            ...
            <div className={`absolute inset-0 rounded-full border transition-all duration-300 pointer-events-none ${
              voiceState === 'listening_for_command' || voiceState === 'transcribing_command'
                ? 'border-rose-400/80 shadow-[0_0_15px_rgba(244,63,94,0.7)]'
                : voiceState === 'speaking_response'
                ? 'border-cyan-400/80 shadow-[0_0_15px_rgba(6,182,212,0.7)]'
                : voiceState === 'processing_command'
                ? 'border-purple-400/80 shadow-[0_0_15px_rgba(168,85,247,0.7)]'
                : 'border-white/20 group-hover/lens:border-cyan-400/50'
            }`} />
          </div>

          {/* Quick Expand & Dismiss Pill */}
          <div className="flex items-center gap-1 bg-black/80 border border-white/15 backdrop-blur-xl px-2 py-1.5 rounded-full shadow-lg">
            <button
            ...
            <button
              onClick={closeJarvis}
              className="p-1 rounded-full text-white/40 hover:text-rose-400 hover:bg-rose-500/10 transition-colors"
              title="Close (Esc)"
            >
              <X size={12} />
            </button>
