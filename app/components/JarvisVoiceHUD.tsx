...
export default function JarvisVoiceHUD() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Ambient Holographic Glow & HUD Grid Background */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[650px] h-[650px] bg-gradient-to-tr from-cyan-500/15 via-purple-500/10 to-rose-500/5 rounded-full blur-[140px] pointer-events-none z-0" />
        <div className="absolute inset-0 bg-[radial-gradient(#ffffff08_1px,transparent_1px)] [background-size:32px_32px] pointer-events-none opacity-40 z-0" />
        ...
        <div className="w-full flex items-center justify-between pb-4 border-b border-white/10 max-w-6xl mx-auto relative z-20">
          ...
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-cyan-500/20 via-black to-purple-500/20 border border-cyan-400/30 flex items-center justify-center shadow-[0_0_20px_rgba(6,182,212,0.3)]">
              <Bot size={20} className="text-cyan-300 animate-pulse" />
            </div>
            <div className="flex flex-col">
              <h1 className="font-heading font-extrabold text-2xl md:text-3xl text-white tracking-tighter lowercase leading-none flex items-center gap-1">
                jarvis<span className="text-cyan-400">.</span>os
              </h1>
              <span className="text-[8px] font-mono uppercase tracking-[0.3em] text-white/40 mt-1">Autonomous Neural Core</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500/20 to-blue-500/20 hover:from-cyan-500/30 hover:to-blue-500/30 text-cyan-300 border border-cyan-400/50 hover:border-cyan-400 text-[10px] font-mono uppercase font-bold tracking-wider transition-all shadow-[0_0_16px_rgba(6,182,212,0.3)] hover:scale-105 active:scale-95 cursor-pointer"
              title="Start Live Voice Conversation (Shift+L)"
            >
              <Radio size={12} className="text-cyan-400 animate-pulse" />
              <span>Live Mode</span>
              <span className="hidden md:inline px-1.5 py-0.2 rounded bg-cyan-400/20 text-[8px] text-cyan-200">⇧L</span>
            </button>
            ...
            <button
              ...
              title="Conversation Telemetry Log"
            >
        ...
        <div className="flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto w-full my-4 relative">
          ...
          {/* Concentric Holographic HUD Rings around Visualizer */}
          <div className="relative flex items-center justify-center mb-6">
            <div className="absolute w-72 sm:w-96 h-72 sm:h-96 rounded-full border border-cyan-500/15 animate-spin pointer-events-none [animation-duration:40s]" />
            <div className="absolute w-60 sm:w-80 h-60 sm:h-80 rounded-full border border-dashed border-purple-500/20 pointer-events-none animate-spin [animation-duration:25s] [animation-direction:reverse]" />
            <div className="absolute w-48 sm:w-64 h-48 sm:h-64 rounded-full border border-white/5 pointer-events-none" />
            ...
            <AnimatePresence mode="wait">
              {(telemetry?.interimTranscript || telemetry?.transcript) ? (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="px-6 py-3 rounded-2xl bg-black/60 border border-cyan-400/30 backdrop-blur-xl shadow-[0_0_30px_rgba(6,182,212,0.2)] font-mono text-lg md:text-2xl text-white font-extrabold tracking-tight"
                >
                  “{telemetry?.interimTranscript || telemetry?.transcript}”
                </motion.div>
                ...
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -10 }}
                  className="px-6 py-3 rounded-2xl bg-black/40 border border-white/10 backdrop-blur-md font-heading font-medium text-lg md:text-2xl text-white/90 leading-relaxed max-w-2xl lowercase tracking-tight"
                >
                ...
                >
                  ...
                  <span className="text-[10px] font-mono text-cyan-400/80 uppercase tracking-widest">
                    Ready for focus timers, tasks, video search and atmosphere streams
                  </span>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          {!showHistory && messages.length > 0 && messages[messages.length - 1]?.mediaResults && (messages[messages.length - 1].mediaResults?.length || 0) > 0 && (
            <motion.div
              ...
              transition={{ duration: 0.35, ease: "easeOut" }}
              className="w-full max-w-4xl mx-auto my-4 p-5 rounded-3xl bg-zinc-950/85 border border-red-500/40 backdrop-blur-3xl space-y-4 z-30 shadow-[0_20px_60px_rgba(0,0,0,0.9),0_0_40px_rgba(239,68,68,0.2)] relative overflow-hidden"
            >
              {/* Red ambient highlight line */}
              <div className="absolute inset-x-8 top-0 h-[1px] bg-gradient-to-r from-transparent via-red-500/60 to-transparent pointer-events-none" />
              ...
              <div className="flex items-center justify-between border-b border-white/10 pb-3 px-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="px-2 py-0.5 rounded-full bg-red-500/20 text-red-300 border border-red-500/30 text-[9px] font-mono font-bold">
                    Click to launch in The Place
                  </span>
                </div>
                <span className="text-[10px] font-mono text-white/40">
                  {messages[messages.length - 1].mediaResults?.length} videos
                </span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3.5 max-h-[42vh] overflow-y-auto no-scrollbar pr-1">
                {messages[messages.length - 1].mediaResults?.map((item) => (
                  <div
                    key={item.id}
                    onClick={() => handleWatchVideoInThePlace(item)}
                    className="group bg-black/60 hover:bg-zinc-900 border border-white/10 hover:border-red-500/60 rounded-2xl p-3 cursor-pointer transition-all duration-300 flex flex-col justify-between space-y-2.5 hover:shadow-[0_10px_30px_rgba(239,68,68,0.25)] hover:scale-[1.02]"
                  >
                    <div className="relative aspect-video rounded-xl overflow-hidden bg-zinc-900 border border-white/5">
                      ...
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-[2px]">
                        <div className="w-11 h-11 rounded-full bg-red-600 text-white flex items-center justify-center shadow-[0_0_20px_rgba(239,68,68,0.8)] transform group-hover:scale-110 transition-transform">
                    ...
                    <div className="space-y-1">
                      ...
                      <h5 className="font-mono text-xs font-bold text-white line-clamp-2 leading-snug group-hover:text-red-200 transition-colors">
                        {item.title}
                      </h5>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-2 px-3 rounded-xl bg-gradient-to-r from-red-600/20 to-rose-600/20 hover:from-red-600 hover:to-rose-600 text-red-300 hover:text-white border border-red-500/40 text-[9px] font-mono font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition-all duration-300 shadow-sm cursor-pointer"
                    >
                      <Play size={10} fill="currentColor" /> Stream in The Place
                    </button>
                  </div>
                ))}
              </div>
                ...
                <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 hover:text-white text-[9px] font-mono uppercase tracking-wider transition-all border border-white/10"
                    >
                      Clear Log
                    </button>
                    <button
                    ...
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border text-xs font-mono transition-all ${
                        m.role === 'user' 
                          ...
                          : 'bg-black/70 border-cyan-500/30 text-cyan-100 mr-auto max-w-[85%] shadow-[0_0_20px_rgba(6,182,212,0.1)]'
                      }`}
                    >
                      <div className="flex items-center justify-between text-[8px] text-white/40 uppercase tracking-widest mb-1.5 font-bold">
                        <span className={m.role === 'user' ? 'text-white/60' : 'text-cyan-400'}>
                          {m.role === 'user' ? 'Direct Vocal Input' : 'J.A.R.V.I.S. Core'}
                        </span>
        ...
        <div className="w-full max-w-3xl mx-auto flex flex-col items-center gap-3.5 relative z-20">
          {/* Main Input Row */}
          <div className="w-full flex items-center gap-2.5 bg-zinc-950/80 border border-white/15 focus-within:border-cyan-400/60 focus-within:shadow-[0_0_30px_rgba(6,182,212,0.25)] p-2 pl-3 rounded-2xl md:rounded-full backdrop-blur-2xl transition-all duration-300 shadow-[0_20px_50px_rgba(0,0,0,0.85)]">
            ...
            {(voiceState === 'LISTENING_FOR_COMMAND' || voiceState === 'PROCESSING_COMMAND' || voiceState === 'SPEAKING_RESPONSE') && (
              <button
                ...
                className="w-8 h-8 rounded-full bg-rose-500/15 hover:bg-rose-500/25 text-rose-300 border border-rose-500/30 flex items-center justify-center transition-all cursor-pointer shrink-0"
          ...
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar max-w-full py-1">
            {[
              ...
              { label: "start 25m timer", icon: <Clock size={10} className="text-cyan-400" /> },
              { label: "add task: review project roadmap", icon: <CheckSquare size={10} className="text-emerald-400" /> },
              { label: "what are my tasks?", icon: <FileText size={10} className="text-yellow-400" /> },
              ...
              { label: "give me a pep talk", icon: <Zap size={10} className="text-amber-400" /> }
            ].map((chip, idx) => (
              <button
                key={idx}
                onClick={() => handleUserMessage(chip.label)}
                className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-full bg-white/[0.04] hover:bg-white/[0.12] border border-white/10 hover:border-cyan-400/40 text-[10px] font-mono text-white/65 hover:text-white transition-all whitespace-nowrap backdrop-blur-md cursor-pointer hover:scale-105 active:scale-95"
              >
                {chip.icon}
